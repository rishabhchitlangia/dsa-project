...
int checksmall(int board[9][3][3], int k) {             //checking sub-board
...
int checkfull(int board[9][3][3], int k) { //checking if the sub-board has been completely filled
...
int checkbig() {             //checking big-board
	...
	for (int z = 0; z < ROWS - 1; z++) {
		...
		c1 = b[z];
		c2 = e[z];
		// main logic to check if the big-board has a winner
		if (big[r1][c1] != 1 &&
...
MoveStatus validmove(int board[9][3][3], int k, int r, int c) {         //checking validity
	if (board[k][r][c] != 1 && r < 3 && c < 3) return SPACE_OCCUPIED; 
	else if (r > 3 || c > 3)
...
void init(int board[9][3][3]) { //initializing boards
